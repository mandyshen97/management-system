...
class TextAnalysis extends Component {
  ...
  callDownload(record_id) {
    window.location.href =
      "http://localhost:8081/record/download?id=" + record_id;
  }
  medicineHelp() {
    this.setState({
      helpSwitch: true,
  ...
  // 处理开始分析按钮
  async handleAnalyse(record) {
    let text =
      record.chfCmp + "。" + record.hisPreIll + "。" + record.prvMedHis;
    await this.callAnalysis(text);
  }
  callAnalysis(record) {
    let param = {
      record: record,
    };
    API.getAnalyseResult(param)
      .then((response) => {
    ...
    return (
      <div className="main-content">
        ...
        <Row justify="space-between">
          ...
          <Col span={4}>
            ...
            <span style={{ marginLeft: 15 }}>
              {this.state.medRecord.doctorName}
            </span>
          </Col>
          <Col span={3}>
            <strong>性别:</strong>
          ...
          <Col span={3}>
            ...
            <span style={{ marginLeft: 15 }}>
              {this.getDisease(
                this.state.medRecord.analysisStatus,
                this.state.medRecord.diagnosticResult
              )}
            </span>
          </Col>
          <Col span={2}>
            <Button
              type="primary"
              onClick={() => this.handleAnalyse(this.state.medRecord)}
            >
              开始分析
            </Button>
          </Col>
        ...
        <Row justify="space-between">
          ...
          <Col span={10}>{this.state.medRecord.chfCmp}</Col>
          <Col span={10} style={{ marginLeft: 15 }}>
            {_.get(this.state.simMedRecord, "chfCmp")}
          </Col>
        </Row>
        <br />
        <Row justify="space-between">
          ...
          <Col span={10}>{_.get(this.state.medRecord, "hisPreIll")}</Col>
          <Col span={10} style={{ marginLeft: 15 }}>
            {" "}
            {_.get(this.state.simMedRecord, "hisPreIll")}
          </Col>
        </Row>
        <br />
        <Row justify="space-between">
          ...
          <Col span={10} style={{ marginLeft: 15 }}>
            {_.get(this.state.simMedRecord, "prvMedHis")}
          </Col>
        </Row>
        <br />
        <Row justify="space-between">
          ...
          <Col span={10} style={{ marginLeft: 15 }}>
            {_.get(this.state.simMedRecord, "tcmType")}
          </Col>
        </Row>
        <br />
        <Row justify="space-between">
          ...
          <Col span={10} style={{ marginLeft: 15 }}>
            {_.get(this.state.simMedRecord, "patientSign")}
          </Col>
        </Row>
        <br />
        <Row justify="space-between">
          ...
          <Col span={10} style={{ marginLeft: 15 }}>
            {_.get(this.state.simMedRecord, "westernMedicine")}
          </Col>
        </Row>
        <Row justify="space-between">
          <Col span={2}>
          ...
          <Col span={10} style={{ marginLeft: 15 }}>
            {_.get(this.state.simMedRecord, "chineseMedicine")}
          </Col>
        </Row>
        <Divider />
        <b>治疗建议</b>
        ...
        <Row>
          <Col>
            ...
            <div className="setformat">
              {_.get(this.state.medRecord, "treAdv")}{" "}
            </div>
          </Col>
        </Row>
        <Modal
        ...
        >
          <p>
            ...
            <br />
            <span style={{ color: "red", margin: "2px 8px 2px 10px" }}>
              柴胡(1.00)
            </span>
